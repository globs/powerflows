...
class SecretsManager(object):
    ...
    def checkIfConfigExistsOrInit(self):
        ...
        logging.info(f'#config results found for db: {tst_minio_secret}, cos: {tst_pg_secret}')
        if  tst_minio_secret['size'] == 0: 
            logging.info(f'Adding internal minio cos credentials to secrets, res for debug : {tst_minio_secret}')
            ...
            logging.info('Minio secret already exists, init minio cos skipped')
        if  tst_pg_secret['size'] == 0: 
            logging.info('Adding internal pg db credentials to secrets')
            ...
            logging.info('internal pg db secret already exists, init skipped')
    ...
    def pushSecretJson(self, secret):
        logging.info(f"Received json secret type {type(secret)}: {secret}")
        if type(secret) == dict:
        ...
        logging.info(f"pushing secret to db: {str_secret}")
    ...
    def getAllSecrets(self):
        ...
        for row in res:
            logging.info(f'Secret display: {row[0]}: {row[1]}')
    ...
    def sqlite_execute_query(self, sql_query, withresults=False, returntype='array'):
        ...
        if withresults:
            res = data.fetchall()
            logging.info(f'Query results : {res}')
